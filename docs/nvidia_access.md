...
### Полный сценарий Z6 после исправлений AI-слоя

На отдельной временной БД с seed проверены точные ответы из `docs/demo_script.md`:
анализ 10,535 с, сборка 8,551 с, оба ответа `ai_mode=nvidia`.
Рейтинг 10 → 65 → 90, публикация #4 → #2, рекомендации DataCats,
ручной выбор/отклонение и начисление +10 за этап работают.
Одна проверка точного оформления выявила отличие: контакт содержит email вместе
с текстом о созвоне, хотя формат связи теперь также выделен в отдельное поле.
Это не меняет рейтинг; человек может оставить в поле контакта только email.
Рабочая БД в этом прогоне не менялась. Прогон через браузер ещё нужен.
