...
- После запуска пользователь прислал скрин успешного запроса внутри Brev:
  ...
  JSON карточки, evidence и время выполнения Z0 ещё не проверены на этой модели.
...
провайдеров. Проверка изменённого клиента через SSH-туннель ещё предстоит.
...
Качество русского JSON, evidence и задержку менее 5 секунд ещё нужно измерить.
...
- Команда запуска публикует API на `127.0.0.1:8000` машины Brev.
  Локальный компьютер ещё не подключён: ожидаются Endpoint и Public Port
  SSH (Destination Port 22) со страницы Access.
- На Windows есть `ssh.exe`, Brev CLI не найден. Для штатного Brev CLI
  документация предусматривает WSL; рассматриваем прямой OpenSSH-туннель.
- Локальный `.env` пока не переключён на сервер Brev.

## Что делать сейчас

Для hosted Build: отправить подготовленное обращение в официальный канал поддержки.
Для собственного сервера: получить адрес SSH, настроить вход по ключу и туннель
`127.0.0.1:9000` на Windows → `127.0.0.1:8000` в Brev. После проверки связи
настроить `.env` и проверить реальные analyze/build и задержку Z0.
