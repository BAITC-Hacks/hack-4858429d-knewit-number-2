...
## Дизайн
- Тема только через src/theme.ts (ConfigProvider). Шрифт Onest, числа tabular-nums.
- Цвета уровней (использовать везде одинаково через LevelTag/ScoreRing):
  draft 
#8A94A6, working 
#3B82F6, ready 
#10A37F, priority 
...
#1B2440, primary 
#3B5BDB.
- Главный акцент — рейтинг и позиция. Остальное спокойно: без градиентов, без КАПСА, без лишних теней.
- Конструктор: 2 колонки, справа sticky-панель рейтинга. Каталог: список-рейтинг с номером места слева, auto-animate.
- Под полем от ИИ — EvidenceNote: «Из вашего текста: «…»».
- Анимации только в ответ на действия. Тексты в обычном регистре, кнопки называют действие.
- Не добавлять UI-библиотеки кроме antd и @formkit/auto-animate. Повторяющийся UI — в src/ui.

после этого промпта какой дальше путь